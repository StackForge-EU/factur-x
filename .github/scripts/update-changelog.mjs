...
if (!newVersion || !oldVersion || !repoUrl) {
  console.error("Usage: node update-changelog.mjs <new-version> <old-version> <repo-url>");
  process.exit(1);
}
...
const notesMatch = changelog.match(/## \[Unreleased\]\n([\s\S]*?)(?=\n## \[|\n\[Unreleased\]:)/);
const notes = notesMatch?.[1]?.trim();

if (!notes) {
  console.error("Error: [Unreleased] section is empty. Add changelog entries before releasing.");
  process.exit(1);
}
